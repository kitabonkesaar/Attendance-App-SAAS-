          ...
          {capturedImage ? (
            ...
            <>
              <video 
                ...
                playsInline 
                muted
                onLoadedMetadata={() => videoRef.current?.play().catch(e => console.warn("Auto-play blocked:", e))}
                className="w-full h-full object-cover scale-x-[-1]" 
              />
              {/* Switch Camera Button (Only visible if camera is active) */}
              <button 
                  onClick={switchCamera}
                  className="absolute bottom-4 right-4 p-3 bg-black/50 backdrop-blur-md rounded-full text-white hover:bg-black/70 transition-all z-20"
                  title="Switch Camera"
              >
